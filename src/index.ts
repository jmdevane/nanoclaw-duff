...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  // Telegram typing indicator expires after 5s — refresh every 4s while working
  ...
  try {
    output = await runAgent(group, prompt, chatJid, async (result) => {
      ...
      if (result.status === 'success') {
        ...
        // Agent is done — stop typing immediately rather than waiting for container to close
        await stopTyping();
      }
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          const slashResult = await handleSlashCommand(messagesToSend, group);
          if (slashResult !== null) {
            await channel.sendMessage(chatJid, slashResult);
            lastAgentTimestamp[chatJid] =
              messagesToSend[messagesToSend.length - 1].timestamp;
            saveState();
            continue;
